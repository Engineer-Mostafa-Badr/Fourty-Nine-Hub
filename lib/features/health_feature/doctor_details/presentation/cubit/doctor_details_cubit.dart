...
// import 'package:fourtyninehub/features/health_feature/doctor_details/domain/usecases/get_doctor_details_Id_usecase.dart'; // OLD API
// import 'package:fourtyninehub/features/health_feature/doctor_details/domain/usecases/get_doctor_details_usecase.dart'; // OLD API
...
import 'package:fourtyninehub/features/health_feature/doctor_details/domain/usecases/get_booking_doctor_by_id_usecase.dart';
import 'package:fourtyninehub/features/health_feature/doctor_details/domain/usecases/get_doctor_availabilities_usecase.dart';
...
class DoctorDetailsCubit extends Cubit<DoctorDetailsState> {
  ...
  // final GetDoctorDetailsUseCase _getDoctorDetailsUseCase; // OLD API
  // final GetDoctorDetailsIdUseCase _getDoctorDetailsIdUseCase; // OLD API
  final AddDoctorRatingUseCase _addDoctorRatingUseCase;
  final GetDoctorReviewsUseCase _getDoctorReviewsUseCase;
  final GetBookingDoctorByIdUseCase _getBookingDoctorByIdUseCase;
  final GetDoctorAvailabilitiesUseCase _getDoctorAvailabilitiesUseCase;
  ...
  DoctorDetailsCubit(
      ...
      // this._getDoctorDetailsUseCase, // OLD API
      // this._getDoctorDetailsIdUseCase, // OLD API
      this._addDoctorRatingUseCase,
      this._getDoctorReviewsUseCase,
      this._getBookingDoctorByIdUseCase,
      this._getDoctorAvailabilitiesUseCase)
  ...
  Future<void> loadData(DoctorDetailsParams params) async {
    emit(state.copyWith(status: DoctorDetailsStates.loading));
    // Prefer new booking doctor details endpoint ONLY (OLD API fallbacks removed)
    final bookingDetails = await _getBookingDoctorByIdUseCase(
      GetBookingDoctorByIdParams(doctorId: params.doctorId),
    );
    bookingDetails.fold(
      (failure) {
        var currentContext =
    ...
    if (currentDoctor != null) {
      final avails = await _getDoctorAvailabilitiesUseCase(
      ...
      avails.fold(
        (failure) {
          // ignore silently; keep existing appointments if any
        },
        (appointments) {
  ...
  // Future<void> _getDoctorDetails(DoctorDetailsParams params) async { // OLD API
  ...
  // Future<void> _getDoctorDetailsId(DoctorDetailsParams params) async { // OLD API
  //   print("objectparams.type${params.type}");
  //
  //   final response =
