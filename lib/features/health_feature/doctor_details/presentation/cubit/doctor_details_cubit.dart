...
class DoctorDetailsCubit extends Cubit<DoctorDetailsState> {
  ...
  Future<void> loadData(DoctorDetailsParams params) async {
    ...
    params.fromSearch==false?await _getDoctorDetailsId(params):await _getDoctorDetails(params);
  ...
  Future<void> _getDoctorDetails(DoctorDetailsParams params) async {
    ...
    final response = await _getDoctorDetailsUseCase.call(GetDoctorDetailsParams(
      ...
      subCategoryId: (params.subCategoryId!=null&&params.subCategoryId!.isNotEmpty)?(params.subCategoryId??''):_healthSharedData.doctorSearchParams.subCategory.id,
    ));
    response.fold(
  ...
  Future<void> fetchDoctorReviews() async {
    ...
    final response = await _getDoctorReviewsUseCase.call(PaginationParams(page: currentPage, limit: pageSize));
  ...
  Future<void> fetchReviews({required String doctorId ,int? page, int? limit}) async {
    print("objectHelloooooo$doctorId");
    if (!hasMoreData || isLoadingMore) return;

    isLoadingMore = true;

    final response = await _getUserDoctorRatessUseCase.call(GetUserDoctorRatesParams(doctorId: doctorId, page: page ?? currentPage, limit: limit??pageSize));
  ...
  Future<void> fetchDoctorSubReviews({required String doctorId}) async {
    ...
    final response = await _getUserDoctorRatessUseCase.call(GetUserDoctorRatesParams(doctorId: doctorId, page: currentPage, limit: pageSize));
