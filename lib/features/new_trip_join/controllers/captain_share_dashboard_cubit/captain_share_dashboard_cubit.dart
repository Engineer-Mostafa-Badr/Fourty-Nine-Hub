...
class CaptainShareDashboardCubit extends Cubit<CaptainShareDashboardState> {
  ...
  CaptainShareDashboardCubit(
      ...
      this.clientNotShownUseCase,
      this.dropClientUseCase,
      this.getDriverPastBookingsUseCase,
      this.pickClientUseCase,
      this.getDriverRunningRouteUseCase,
      this.acceptRouteUseCase,
      this.getDriverAvailableBookingsUseCase,
  ...
  Future<void> acceptRoute(
    ...
    response.fold((l) {
      ...
      emit(state.copyWith(
          failure: l, status: CaptainShareDashboardStates.error));
    }, (data) {
      context.pop();
  ...
  changeTapIndex(int index, BuildContext context) {
    ...
    emit(state.copyWith(
        tapIndex: index, status: CaptainShareDashboardStates.success));
  }
  ...
  Future<void> dropOffClient(
    ...
    response.fold((l) {
      ...
      showSuccessMessage(
          context,
          context.isArabic
              ? 'تم توصيل الراكب بنجاح'
              : 'Client Dropped Off Successfully');
      emit(state.copyWith(status: CaptainShareDashboardStates.success));
    });
  }
  ...
  Future<void> getAvailableBookings(BuildContext context) async {
    ...
    response.fold(
      (failure) {
        ...
        emit(state.copyWith(
            failure: failure, status: CaptainShareDashboardStates.error));
      },
      (data) {
  ...
  BookingClientEntity? getCurrentClient(List<BookingClientEntity> clients) {
    clients.sort((a, b) => (a.pickupDistanceFromStart ?? 0)
        .compareTo(b.pickupDistanceFromStart ?? 0));
    BookingClientEntity? currentClient;
    if (clients.isNotEmpty &&
        (clients[0].status == RouteClientStatus.acceptedByDriver.name ||
            clients[0].status ==
                RouteClientStatus.driverNoShowPassenger.name)) {
      currentClient = clients[0];
    } else if (clients.length > 1 &&
        (clients[1].status == RouteClientStatus.acceptedByDriver.name ||
            clients[1].status ==
                RouteClientStatus.driverNoShowPassenger.name)) {
      currentClient = clients[1];
    } else if (clients.length >= 2 &&
        (clients[2].status == RouteClientStatus.acceptedByDriver.name ||
            clients[2].status ==
                RouteClientStatus.driverNoShowPassenger.name)) {
  ...
  Future<void> getPastBookings(BuildContext context) async {
    ...
    final response = await getDriverPastBookingsUseCase(
      PaginationParams(limit: 10, page: currentPastPage),
    );
  ...
  Future<void> getRunningRoute(BuildContext context) async {
    ...
    runningRoute?.status = '';
    emit(state.copyWith(
        status: CaptainShareDashboardStates.loading,
    ...
    response.fold(
      ...
      (data) {
        isLoadingRunningTrip = false;
        emit(state.copyWith(
            status: CaptainShareDashboardStates.success, runningRoute: data));
      },
    );
  }
  ...
  Future<void> goToClient(
      ...
      required BuildContext context}) async {
    showLoadingDialog(context);
    final response = await pickClientUseCase(
        PickClientParams(routeId: routeId, passengerId: passengerId, otp: otp));
    response.fold((l) {
      var currentContext =
          AppPages.router.configuration.navigatorKey.currentContext!;
      showErrorMessage(currentContext, getFailureMessage(l, currentContext));
      ...
      showSuccessMessage(
          context,
          context.isArabic
              ? 'تم التقاط الراكب بنجاح'
              : 'Client Picked Successfully');
      emit(state.copyWith(status: CaptainShareDashboardStates.success));
    });
  }
    ...
    listenToNewRouteDriverUseCase((route) {
      if (state.tapIndex == 0) {
        ...
        showSuccessMessage(context,
            context.isArabic ? 'تم استقبال رحلة جديدة' : 'New route accepted');
      } else {
  ...
  Future<void> onClientNotShown(
    ...
    response.fold((l) {
      ...
      currentContext.pop();
      MyBookingEntity? runningRoute = state.runningRoute;
      runningRoute?.clients
          ?.firstWhereOrNull((e) => e.id == passengerId)
          ?.driverWaitingTime = data;
      emit(state.copyWith(
          status: CaptainShareDashboardStates.success,
          runningRoute: runningRoute));
    });
  ...
  Future<void> onDriverArrivedToClient(
    ...
    response.fold((l) {
      ...
      runningRoute?.clients
          ?.firstWhereOrNull((e) => e.id == passengerId)
          ?.driverArrivalTime = data;
      showSuccessMessage(
          currentContext,
          currentContext.isArabic
              ? 'تم الوصول للراكب بنجاح'
  ...
  Future<void> startClientRoute(
    ...
    showLoadingDialog(context);
    final response = await pickClientUseCase(
        PickClientParams(routeId: id, passengerId: passengerId, otp: otp));
    response.fold((l) {
      var currentContext =
          AppPages.router.configuration.navigatorKey.currentContext!;
      showErrorMessage(currentContext, getFailureMessage(l, currentContext));
      ...
      runningRoute?.clients
          ?.firstWhereOrNull((e) => e.id == passengerId)
          ?.status = RouteClientStatus.pickedUp.name;
