...
class AvailableTripCard extends StatefulWidget {
  const AvailableTripCard({super.key, required this.trip, required this.params});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<DashboardsCubit, DashboardsState>(
      builder: (context,state) {
        ...
        num offer = (widget.trip.offerPriceRange?.lastOffer??0)+3>(widget.trip.offerPriceRange?.highestFare??0)?(widget.trip.offerPriceRange?.highestFare??0):(widget.trip.offerPriceRange?.lastOffer??0);
        print("offer $offer");
        return Card(
          elevation: 4,
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            children: [
                              Text(
                      FormatNumbers().convertNumberToLocalizedString((widget.trip.price??0).ceil().toString(),isArabic:context.isArabic),
                        ...
                        Row(
                          children: [
                            ...
                            ProfilePictureWidget(
                              ...
                              firstChar: (widget.trip.clientDetails?.firstName??' ')[0].toUpperCase(),
                            ),
                          ],
                        ),
                        Row(
                          mainAxisAlignment: MainAxisAlignment.end,
                          children: [
                            if(widget.trip.clientDetails?.isAccountVerified==true)...[Icon(
                              Icons.verified,
                ...
                ClickableWidget(
                  onTap: (){
                    if (_isButtonDisabled||((widget.trip.offerPriceRange?.lastOffer??0)+3>(widget.trip.offerPriceRange?.highestFare??0))) return;
                    ManageVibration.vibrate();
                    if(widget.trip.isPremium==true||widget.trip.state?.isButtonEnabled==true){
                      if (widget.trip.isAutoAccept == false) {
                        cubit.createOffer(tripId: widget.trip.id??'', price: ((widget.trip.offerPriceRange?.lastOffer??0).ceil() + 3), context: context, subCategoryId: widget.trip.subcategory?.id??'', onSuccess: () {
                          widget.trip.offerPriceRange?.lastOffer = ((widget.trip.offerPriceRange?.lastOffer??0).ceil() + 3);
                        ...
                        cubit.autoAcceptTrip(context, widget.trip.id??'', widget.params);
                      }
                    }else{
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          borderRadius: BorderRadius.circular(12),
                          color: (_isButtonDisabled && _remainingTime != null||(((widget.trip.offerPriceRange?.lastOffer??0)+3>(widget.trip.offerPriceRange?.highestFare??0))))?AppColors.GREY_DARK_COLOR:AppColors.PRIMARY_COLOR,
                        ),
                        child: Text(
                          '+${FormatNumbers().convertNumberToLocalizedString('3',isArabic: context.isArabic)}',
                          style: Styles.mediumText(color: AppColors.whiteColor,fontSize: 30),
                        ),
                      ),
                    ),
                    Sizer(width: 8),
                        ...
                        child: SizedBox(
                          ...
                          child: ElevatedButton(
                            onPressed: () {
                              ...
                              if(widget.trip.isPremium==true||widget.trip.state?.isButtonEnabled==true){
                                if (widget.trip.isAutoAccept == false) {
                                  cubit.createOffer(tripId: widget.trip.id??'', price: offer, context: context, subCategoryId: widget.trip.subcategory?.id??'', onSuccess: () {
                                    // Save timer when offer is sent
                                    _saveOfferTimer();
                                    final currentContext = AppPages.router.configuration.navigatorKey.currentContext!;
                                    currentContext.pop();
                            ...
                            style: ElevatedButton.styleFrom(
                              backgroundColor: (_isButtonDisabled && _remainingTime != null)?AppColors.GREY_DARK_COLOR:AppColors.PRIMARY_COLOR,
                    ...
                    ClickableWidget(
                      onTap: (){
                        if (_isButtonDisabled||(((widget.trip.offerPriceRange?.lastOffer??0)-3)<(widget.trip.offerPriceRange?.lowestFare??0))) return;
                        ManageVibration.vibrate();
                        if(widget.trip.isPremium==true||widget.trip.state?.isButtonEnabled==true){
                          if (widget.trip.isAutoAccept == false) {
                            cubit.createOffer(tripId: widget.trip.id??'', price: ((widget.trip.offerPriceRange?.lastOffer??0).ceil() - 3), context: context, subCategoryId: widget.trip.subcategory?.id??'', onSuccess: () {
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          borderRadius: BorderRadius.circular(12),
                          color: (_isButtonDisabled && _remainingTime != null||(((widget.trip.offerPriceRange?.lastOffer??0).ceil()-3)<(widget.trip.offerPriceRange?.lowestFare??0).floor()))?AppColors.GREY_DARK_COLOR:AppColors.PRIMARY_COLOR,
                        ),
                        child: Text(
                          '-${FormatNumbers().convertNumberToLocalizedString('3',isArabic: context.isArabic)}',
                          style: Styles.mediumText(color: AppColors.whiteColor,fontSize: 30),
                    ...
                    Expanded(
                      ...
                      child: ClickableWidget(
                        onTap: (){
                          ...
                          if(widget.trip.isPremium==true||widget.trip.state?.isButtonEnabled==true){
                            if (widget.trip.isAutoAccept == false) {
                              cubit.createOffer(tripId: widget.trip.id??'', price: ((widget.trip.offerPriceRange?.lowestFare??0).ceil() + (((widget.trip.offerPriceRange?.highestFare??0) - (widget.trip.offerPriceRange?.lowestFare??0)) * 0.2)).ceil(), context: context, subCategoryId: widget.trip.subcategory?.id??'', onSuccess: () {
                                // Save timer when offer is sent
                                widget.trip.offerPriceRange?.lastOffer = ((widget.trip.offerPriceRange?.lowestFare??0) + (((widget.trip.offerPriceRange?.highestFare??0).ceil() - (widget.trip.offerPriceRange?.lowestFare??0)) * 0.2)).ceil();
                                _saveOfferTimer();
                                final currentContext = AppPages.router.configuration.navigatorKey.currentContext!;
                                currentContext.pop();
                                toastification.show(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(12),
                            color: _isButtonDisabled && _remainingTime != null?AppColors.GREY_DARK_COLOR:AppColors.PRIMARY_COLOR,
                          ),
                          child: Center(
                              child: Text(
                    ...
                    Expanded(
                      ...
                      child: ClickableWidget(
                        onTap: (){
                          ...
                          print("tripEntity.isButtonEnabled ${widget.trip.state?.isButtonEnabled}");
                          if(widget.trip.isPremium==true||widget.trip.state?.isButtonEnabled==true){
                            if (widget.trip.isAutoAccept == false) {
                              cubit.createOffer(tripId: widget.trip.id??'', price: ((widget.trip.offerPriceRange?.lowestFare??0).ceil() + (((widget.trip.offerPriceRange?.highestFare??0) - (widget.trip.offerPriceRange?.lowestFare??0)) * 0.5)).ceil(), context: context, subCategoryId: widget.trip.subcategory?.id??'', onSuccess: () {
                                widget.trip.offerPriceRange?.lastOffer = ((widget.trip.offerPriceRange?.lowestFare??0) + (((widget.trip.offerPriceRange?.highestFare??0).ceil() - (widget.trip.offerPriceRange?.lowestFare??0)) * 0.5)).ceil();
                                // Save timer when offer is sent
                                _saveOfferTimer();
                                final currentContext = AppPages.router.configuration.navigatorKey.currentContext!;
                                currentContext.pop();
                    ...
                    Expanded(
                      ...
                      child: ClickableWidget(
                        onTap: (){
                          ...
                          if(widget.trip.isPremium==true||widget.trip.state?.isButtonEnabled==true){
                            if (widget.trip.isAutoAccept == false) {
                              cubit.createOffer(tripId: widget.trip.id??'', price: ((widget.trip.offerPriceRange?.lowestFare??0) + (((widget.trip.offerPriceRange?.highestFare??0) - (widget.trip.offerPriceRange?.lowestFare??0)) * 1)).floor(), context: context, subCategoryId: widget.trip.subcategory?.id??'', onSuccess: () {
                                widget.trip.offerPriceRange?.lastOffer = ((widget.trip.offerPriceRange?.lowestFare??0) + (((widget.trip.offerPriceRange?.highestFare??0) - (widget.trip.offerPriceRange?.lowestFare??0)) * 1)).floor();
                                // Save timer when offer is sent
                                _saveOfferTimer();
                                final currentContext = AppPages.router.configuration.navigatorKey.currentContext!;
                                currentContext.pop();
