...
class SettingsWidget extends StatefulWidget {
  ...
  const SettingsWidget(
      {super.key, required this.modeType, required this.params, this.settings});
...
class _SettingsWidgetState extends State<SettingsWidget> {
  ...
  var planController = ExpansibleController();
  var cityController = ExpansibleController();
  ...
  void initState() {
    ...
    if ((widget.settings?.categoryIds.length ?? 0) > 0) {
      isCaptain = widget.settings?.categoryIds[0].isActive ?? false;
    }
    if ((widget.settings?.categoryIds.length ?? 0) > 1) {
      isIntercity = widget.settings?.categoryIds[1].isActive ?? false;
    }
    if ((widget.settings?.categoryIds.length ?? 0) > 2) {
      isPremium = widget.settings?.categoryIds[2].isActive ?? false;
    }
    hasIdRequest = (widget.settings?.requests ?? [])
        .any((e) => e.recordName == RecordStatusEnum.nationalId.status);
    hasDriverLicenseRequest = (widget.settings?.requests ?? [])
        .any((e) => e.recordName == RecordStatusEnum.drivingLicense.status);
    hasCarLicenseRequest = (widget.settings?.requests ?? [])
        .any((e) => e.recordName == RecordStatusEnum.carLicense.status);
    hasCriminalRecordRequest = (widget.settings?.requests ?? [])
        .any((e) => e.recordName == RecordStatusEnum.criminalRecord.status);
    hasDrugAnalysisRequest = (widget.settings?.requests ?? [])
        .any((e) => e.recordName == RecordStatusEnum.drugAnalysis.status);
    hasTechnicalExaminationRequest = (widget.settings?.requests ?? []).any(
        (e) => e.recordName == RecordStatusEnum.technicalExamination.status);
    if ((widget.settings?.requests ?? [])
        .any((e) => e.recordName == RecordStatusEnum.drivingLicense.status)) {
      driverLicenseRequestStatus = widget.settings?.requests
              .firstWhere(
                  (e) => e.recordName == RecordStatusEnum.drivingLicense.status)
              .status ??
          '';
    }
    if ((widget.settings?.requests ?? [])
        .any((e) => e.recordName == RecordStatusEnum.nationalId.status)) {
      idRequestStatus = widget.settings?.requests
              .firstWhere(
                  (e) => e.recordName == RecordStatusEnum.nationalId.status)
              .status ??
          '';
    }
    if ((widget.settings?.requests ?? [])
        .any((e) => e.recordName == RecordStatusEnum.carLicense.status)) {
      carLicenseRequestStatus = widget.settings?.requests
              .firstWhere(
                  (e) => e.recordName == RecordStatusEnum.carLicense.status)
              .status ??
          '';
    }
    if ((widget.settings?.requests ?? [])
        .any((e) => e.recordName == RecordStatusEnum.criminalRecord.status)) {
      criminalRecordRequestStatus = widget.settings?.requests
              .firstWhere(
                  (e) => e.recordName == RecordStatusEnum.criminalRecord.status)
              .status ??
          '';
    }
    if ((widget.settings?.requests ?? [])
        .any((e) => e.recordName == RecordStatusEnum.drugAnalysis.status)) {
      drugAnalysisRequestStatus = widget.settings?.requests
              .firstWhere(
                  (e) => e.recordName == RecordStatusEnum.drugAnalysis.status)
              .status ??
          '';
    }
    if ((widget.settings?.requests ?? []).any(
        (e) => e.recordName == RecordStatusEnum.technicalExamination.status)) {
      technicalExaminationRequestStatus = widget.settings?.requests
              .firstWhere((e) =>
                  e.recordName == RecordStatusEnum.technicalExamination.status)
              .status ??
          '';
    }
  }
  ...
  int calculateDaysUntilExpiry(String expiryDateString) {
    final expiryDate = expiryDateString.isEmpty
        ? DateTime.now().toUtc()
        : DateTime.parse(expiryDateString).toUtc();
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: widget.settings == null
          ? Center(
              child: CustomEmptyWidget(
                  label: context.isArabic ? 'حدث مشكلة' : 'An error occurred'),
            )
          : ListView(
              children: [
                // switchWidget(
                ...
                switchWidget(
                    ...
                    subText:
                        isComfort ? LocaleKeys.on.tr() : LocaleKeys.off.tr(),
                    valuee: isComfort,
                    onChanged: (value) {
                      ManageVibration.vibrate();
                      setState(() {
                ...
                switchWidget(
                    ...
                    subText:
                        isNonSmoking ? LocaleKeys.on.tr() : LocaleKeys.off.tr(),
                    valuee: isNonSmoking,
                    onChanged: (value) {
                      ManageVibration.vibrate();
                      setState(() {
                ...
                if (widget.modeType == 'ride') ...[
                  switchWidget(
                      title:
                          context.isArabic ? 'اشعارات صوتية' : 'Voice notify',
                      subText: enableSound
                          ? context.isArabic
                              ? 'تفعيل'
                              : 'Enabled'
                  ...
                  if ((widget.settings?.categoryIds.length ?? 0) >= 1)
                    switchWidget(
                        title: widget.settings?.categoryIds[0]
                            .pictureUrl, //Assets.greyCar,
                        isText: false,
                  ...
                  if ((widget.settings?.categoryIds.length ?? 0) > 1)
                    switchWidget(
                        title: widget.settings?.categoryIds[1]
                            .pictureUrl, //Assets.greyCar,
                        isText: false,
                        subText: context.isArabic
                  ...
                  if ((widget.settings?.categoryIds.length ?? 0) > 2)
                    switchWidget(
                        title: widget.settings?.categoryIds[2]
                            .pictureUrl, //Assets.greyCar,
                        isText: false,
                        subText: context.isArabic
                  ...
                  _expansionTileWidget(
                    ...
                    title:
                        LocaleKeys.subscriptionPlan.tr(), //'Subscription plan',
                      ...
                      (index) => InkWell(
                        ...
                        child: List.generate(
                            ...
                            (index) => Align(
                                ...
                                child: Label(
                                    text:
                                        subscriptionPlans[index].tr())))[index],
                      ),
                    ),
                  ),
                  _expansionTileWidget(
                    ...
                    childrenList: List.generate(
                      ...
                      (index) => InkWell(
                        onTap: () {
                          ManageVibration.vibrate();
                          context.read<DashboardsCubit>().onSelectGovernorate(
                              context
                                  .read<DashboardsCubit>()
                                  .state
                                  .govs?[index]);
                              ...
                              customBottomSheet2(context,
                                  child: Padding(
                                    ...
                                    child: UpdateFareBottomSheetWidget(
                                      ...
                                      highCostPerKm:
                                          widget.settings?.highCostPerKm ?? 0,
                                      lowCostPerKm:
                                          widget.settings?.lowCostPerKm ?? 0,
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      RatingBar(
                        initialRating:
                            widget.settings?.rating.averageRating ?? 2.5,
                        ignoreGestures: true,
                      ...
                      Text(
                          FormatNumbers().convertNumberToLocalizedString(
                              widget.settings?.rating.totalRatings.toString() ??
                                  '0.0',
                              isArabic: context.isArabic),
                          style: const TextStyle(
                    ...
                    children: [
                      Text(LocaleKeys.totalProfit.tr(), //'Total Profit',
                          style: const TextStyle(
                              fontSize: 14, fontWeight: FontWeight.w500)),
                      Text(
                          '${FormatNumbers().convertNumberToLocalizedString(widget.settings?.profit.toString() ?? '0', isArabic: context.isArabic)} ${LocaleKeys.egp.tr()}',
                          style: const TextStyle(
                              fontSize: 14, fontWeight: FontWeight.w500))
                    ],
                  ),
                ...
                ClickableWidget(
                    onTap: () async {
                      if (hasIdRequest &&
                          (idRequestStatus ==
                              DriverUpdateRequestStatusEnum.PENDING.name)) {
                        return;
                      }
                      ManageVibration.vibrate();
                      await Navigator.of(context).push(MaterialPageRoute(
                          builder: (_) => BlocProvider(
                              ...
                              child:
                                  const PersonalDocumentsNonSocketScreen())));
                      context.read<DashboardsCubit>().getSettings(context);
                    },
                    child: UpdatePersonalInfoWidget(
                        title: LocaleKeys.id.tr(),
                        isEnabled: !(hasIdRequest &&
                            (idRequestStatus ==
                                DriverUpdateRequestStatusEnum.PENDING.name)),
                        exdIn: calculateDaysUntilExpiry(
                            widget.settings?.idExpiryDate ?? ''))),
                if (hasIdRequest &&
                    (idRequestStatus ==
                        DriverUpdateRequestStatusEnum.PENDING.name))
                  Align(
                ...
                ClickableWidget(
                  onTap: () async {
                    if (hasDriverLicenseRequest &&
                        (driverLicenseRequestStatus ==
                            DriverUpdateRequestStatusEnum.PENDING.name)) {
                      return;
                    }
                    ManageVibration.vibrate();
                  ...
                  child: UpdatePersonalInfoWidget(
                      ...
                      isEnabled: !(hasDriverLicenseRequest &&
                          (driverLicenseRequestStatus ==
                              DriverUpdateRequestStatusEnum.PENDING.name)),
                      exdIn: calculateDaysUntilExpiry(
                          widget.settings?.drivingLicenseExpiryDate ?? '')),
                ),
                if (hasDriverLicenseRequest &&
                    (driverLicenseRequestStatus ==
                        DriverUpdateRequestStatusEnum.PENDING.name))
                ...
                if (widget.modeType == 'ride') ...[
                  ClickableWidget(
                    onTap: () async {
                      if (hasCarLicenseRequest &&
                          (carLicenseRequestStatus ==
                              DriverUpdateRequestStatusEnum.PENDING.name)) {
                        return;
                      }
                      ManageVibration.vibrate();
                      await Navigator.of(context).push(MaterialPageRoute(
                          builder: (_) => BlocProvider(
                              create: (context) =>
                                  serviceLocator<DashboardsCubit>(),
                              child:
                                  const VehicleInformationNonSocketScreen())));
                      context.read<DashboardsCubit>().getSettings(context);
                    },
                    child: UpdatePersonalInfoWidget(
                        title: LocaleKeys.carLicense.tr(),
                        isEnabled: !(hasCarLicenseRequest &&
                            (carLicenseRequestStatus ==
                                DriverUpdateRequestStatusEnum.PENDING.name)),
                        exdIn: calculateDaysUntilExpiry(
                            widget.settings?.carLicenseExpiryDate ?? '')),
                  ),
                  if (hasCarLicenseRequest &&
                      (carLicenseRequestStatus ==
                          DriverUpdateRequestStatusEnum.PENDING.name))
                  ...
                  if (widget.settings?.isCriminalRecordEnabled == true) ...[
                    ClickableWidget(
                      onTap: () async {
                        if (hasCriminalRecordRequest &&
                            (criminalRecordRequestStatus ==
                                DriverUpdateRequestStatusEnum.PENDING.name)) {
                          return;
                        }
                        ManageVibration.vibrate();
                      ...
                      child: UpdatePersonalInfoWidget(
                          ...
                          isEnabled: !(hasCriminalRecordRequest &&
                              (criminalRecordRequestStatus ==
                                  DriverUpdateRequestStatusEnum.PENDING.name)),
                          exdIn: 4),
                    ),
                  ...
                  if (widget.settings?.isDrugAnalysisRecordEnabled == true) ...[
                    ClickableWidget(
                      onTap: () async {
                        if (hasDrugAnalysisRequest &&
                            (drugAnalysisRequestStatus ==
                                DriverUpdateRequestStatusEnum.PENDING.name)) {
                          return;
                        }
                        ManageVibration.vibrate();
                      ...
                      child: UpdatePersonalInfoWidget(
                          ...
                          isEnabled: !(hasDrugAnalysisRequest &&
                              (drugAnalysisRequestStatus ==
                                  DriverUpdateRequestStatusEnum.PENDING.name)),
                          exdIn: calculateDaysUntilExpiry(
                              widget.settings?.drugAnalysisExpiryDate ?? '')),
                    ),
                    if (hasDrugAnalysisRequest &&
                        (drugAnalysisRequestStatus ==
                            DriverUpdateRequestStatusEnum.PENDING.name))
                ...
                if (widget.settings?.isVehicleRecordEnabled == true) ...[
                  ClickableWidget(
                    onTap: () async {
                      if (hasTechnicalExaminationRequest &&
                          (technicalExaminationRequestStatus ==
                              DriverUpdateRequestStatusEnum.PENDING.name)) {
                        return;
                      }
                      ManageVibration.vibrate();
                      await Navigator.of(context).push(MaterialPageRoute(
                          builder: (_) => BlocProvider.value(
                              value: serviceLocator<DashboardsCubit>(),
                              child:
                                  const TechnicalExaminationNonSocketScreen())));
                      context.read<DashboardsCubit>().getSettings(context);
                    },
                    child: UpdatePersonalInfoWidget(
                        title: LocaleKeys.vehicleInspection.tr(),
                        isEnabled: !(hasTechnicalExaminationRequest &&
                            (technicalExaminationRequestStatus ==
                                DriverUpdateRequestStatusEnum.PENDING.name)),
                        exdIn: calculateDaysUntilExpiry(
                            widget.settings?.technicalExaminationExpiryDate ??
                                '')),
                  ),
                  if (hasTechnicalExaminationRequest &&
                      (technicalExaminationRequestStatus ==
                                ...
                                UpdateSettingsDashboardUsecaseParam(
                                    ...
                                    favoriteCity: context.isArabic
                                        ...
                                        : context
                                                ...
                                                ?.nameEn ??
                                            '',
                                    subCategoriesActive: List.generate(
                                        widget.settings?.categoryIds.length ??
                                            0,
                                        (index) => SubCategoriesActive(
                                            subcategoryId: widget.settings!
