...
class _UploadRiderImagesState extends State<UploadRiderImages> {
    ...
    return PopScope(
      ...
      child: RefreshIndicator(
        onRefresh: () => context
            ...
            .fetchRideUploadedImagesData(context, widget.params),
        child: Scaffold(
  ...
  Widget loadingUploadImages(RideRegisterCubit cubit, RideRegisterState state,
      UploadRiderImagesParams params) {
    return Column(children: [
      ...
      UploadImageRow(
        title: LocaleKeys.personalDocuments.localize,
        onTap: () async {
          ManageVibration.vibrate();
          await Navigator.of(context).push(MaterialPageRoute(
              builder: (_) => BlocProvider.value(
      ...
      UploadImageRow(
        title: LocaleKeys.driversLicense.localize,
        onTap: () async {
          ManageVibration.vibrate();
          await Navigator.of(context).push(MaterialPageRoute(
              builder: (_) => BlocProvider.value(
  ...
  Widget rideUploadImages(
      RideRegisterState state, UploadRiderImagesParams params) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (state.pictureOptional != null &&
          state.pictureOptional?.openCriminalRecord == true &&
        ...
        UploadImageRow(
          ...
          disableUpload: (state.isUploadCriminalRecord == true),
        ),
        const SizedBox(
          height: 40,
        )
      ...
      if (state.pictureOptional != null &&
          state.pictureOptional?.openDrugAnalysis == true &&
          state.registerType == 'socket') ...[
        UploadImageRow(
        ...
        if (state.pictureOptional?.drugAnalysisPhone.isNotEmpty ?? false) ...[
          const SizedBox(
            height: 10,
          ),
          RichText(
        ...
        if (state.pictureOptional?.drugAnalysisAddress.isNotEmpty ?? false) ...[
          const SizedBox(
            height: 10,
          ),
          RichText(
      ...
      if (state.pictureOptional != null &&
          state.pictureOptional?.openTechnicalExamination == true &&
          state.registerType == 'socket') ...[
        UploadImageRow(
        ...
        if (state.pictureOptional?.technicalExaminationPhone.isNotEmpty ??
            false) ...[
          const SizedBox(
            height: 10,
          ),
        ...
        if (state.pictureOptional?.technicalExaminationAddress.isNotEmpty ??
            false) ...[
          const SizedBox(
            height: 10,
          ),
          RichText(
            text: TextSpan(children: [
              ...
              TextSpan(
                  text:
                      state.pictureOptional?.technicalExaminationAddress ?? '',
                  style: Styles.mediumText(
                      color: context.isDarkMode
                          ? Colors.white
                          : AppColors.SECONDARY_COLOR))
