...
import 'package:fourtyninehub/features/social_media/chat/chat_room/presentation/pages/camera_picker/video_editor/src/controller/controller.dart';
import 'package:fourtyninehub/features/social_media/chat/chat_room/presentation/pages/camera_picker/video_editor/src/enums/story_editing_modes.dart';
import 'package:fourtyninehub/features/social_media/chat/chat_room/presentation/pages/camera_picker/video_editor/src/models/stroke.dart';
import 'package:fourtyninehub/features/social_media/chat/chat_room/presentation/pages/camera_picker/video_editor/src/utils/utils.dart';
import 'package:fourtyninehub/features/social_media/chat/chat_room/presentation/pages/camera_picker/video_editor/src/views/main_control_views/filters_view.dart';
import 'package:fourtyninehub/features/social_media/chat/chat_room/presentation/pages/camera_picker/video_editor/src/widgets/draggable_sticker_widget.dart';
import 'package:fourtyninehub/features/social_media/chat/chat_room/presentation/pages/camera_picker/video_editor/src/widgets/draggable_text_widget.dart';
...
class _MainControlsViewState extends State<MainControlsView> {
  ...
  void generateVideoFilesThumbnails() async {
    for (File file in widget.selectedFiles ?? []) {
      if (file.isVideo || file.isTemp) {
        var generatedThumbnail = await generateThumbnail(path: file.path);
        if (mounted) {
          setState(() {
  ...
  void initState() {
    ...
    for (var file in widget.selectedFiles!) {
      ...
      if ((file.isVideo || file.isTemp) && sizeInMB > maxVideoSizeMB) {
        Navigator.pop(context);
      }
// !file.isVideo
      if (file.isImage && sizeInMB > maxImageSizeMB) {
        Navigator.pop(context);
      }
    }
      ...
      child: Column(
        children: [
          ...
          if(widget.isFocused == false)
            widget.selectedFiles![widget.currentPageIndex].isVideo||widget.selectedFiles![widget.currentPageIndex].isTemp
              ? Container()
