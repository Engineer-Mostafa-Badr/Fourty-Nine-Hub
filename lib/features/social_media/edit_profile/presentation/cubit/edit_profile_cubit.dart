...
class EditProfileCubit extends Cubit<EditProfileState> {
  ...
  EditProfileCubit(this._editProfileUseCase, this.getSettingsDashboardUsecase,
    ...
    if (state.isDriverLady == true && params.isMale == true) {
      ...
      showErrorMessage(
          ...
          currentContext.isArabic
              ? 'لا يمكن تغيير جنسية السائقين. يرجى الاتصال بالدعم'
              : 'Drivers are not allowed to change their gender. Please contact support');
      return;
    }
  ...
  Future<void> getSettings(BuildContext context) async {
    ...
    result.fold(
      ...
      (settings) {
        ...
        emit(state.copyWith(isDriverLady: isDriverLady));
      },
    );
  }
