...
class _BuildReactionsButtonsState extends State<BuildReactionsButtons> {
    ...
    return BlocBuilder<SocialPostsCubit, SocialPostsState>(
      ...
      return ReactionButton<String>(
        ...
        onReactionChanged: (Reaction<String>? reaction) async {
          if (reaction?.value == 'likes' ||
              reaction?.value == 'like' && widget.post.isLikes == false) {
            var response = widget.from == 'posts'
                ? await controller.onReact(
                    params: PostReactParams(
                        postId: widget.post.id,
                        react: reaction?.value ?? 'like'),
                    from: widget.from)
          ...
          } else if (reaction?.value == 'likes' ||
              reaction?.value == 'like' && widget.post.isLikes == true) {
            var response = widget.from == 'posts'
                ? await controller.onReact(
                    params: PostReactParams(
                        postId: widget.post.id,
  ...
  Widget _buildReactionItem(
      {required Reactions item, required num count, String? from}) {
    return Row(
