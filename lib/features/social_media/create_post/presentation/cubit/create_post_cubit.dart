...
import '../../domain/usecases/get_live_event_categories_usecase.dart';
import '../../domain/usecases/get_live_event_sub_categories_usecase.dart';
...
class CreatePostCubit extends Cubit<CreatePostState> {
  ...
  void createPost({
    ...
    if (postContentTextController.text.isNotEmpty ||
      ...
      final response = await _createPostUseCase(
        PostParams(
          type: (state.selectedLifeEvent != null &&
                  (state.selectedLifeEvent?.id.isNotEmpty ?? false))
              ? "live_event_post"
              : (state.gifImage != null &&
  ...
  fetchFriendsFollowers(String Search) async {
    ...
    final response = await _friendsFollowersUseCase(FriendsFollowersParams(
        search: searchController.text, limit: 25, page: usersPage));
    response.fold(
      (l) {
        var currentContext =
      ...
      (data) {
        ...
        for (var user in usersList) {
          user.isSelected = selectedUserList
  ...
  Future<void> getActivities() async {
    ...
    emit(state.copyWith(status: CreatePostStates.loading));
    final response = await _getActivitiesUseCase(
        PaginationParams(page: activitiesPage, limit: 20));
    response.fold((l) {
      var currentContext =
          AppPages.router.configuration.navigatorKey.currentContext!;
      showErrorMessage(currentContext, getFailureMessage(l, currentContext));
  ...
  Future<void> getFeelings() async {
    ...
    final response = await _getFeelingsUseCase(
        PaginationParams(page: feelingsPage, limit: 20));
    response.fold((l) {
      var currentContext =
          AppPages.router.configuration.navigatorKey.currentContext!;
      showErrorMessage(currentContext, getFailureMessage(l, currentContext));
  ...
  Future<void> getSubActivities(String id) async {
    ...
    emit(state.copyWith(status: CreatePostStates.loading));
    final response = await _getSubActivitiesUseCase(
        GetSubActivitiesParams(page: activitiesPage, limit: 20, id: id));
    response.fold((l) {
      var currentContext =
          AppPages.router.configuration.navigatorKey.currentContext!;
      showErrorMessage(currentContext, getFailureMessage(l, currentContext));
  ...
  bool handlePopAction() {
    if ((state.images?.isNotEmpty ?? false) ||
        (state.gifImage != null && (state.gifImage?.isNotEmpty ?? false)) ||
        (state.selectedLifeEvent != null &&
            (state.selectedLifeEvent?.id.isNotEmpty ?? false)) ||
        postContentTextController.text.isNotEmpty ||
        (state.backColor != '#FFFFFFFF' && state.backColor != null) ||
        (state.selectedActivity != null &&
            (state.selectedActivity?.id.isNotEmpty ?? false)) ||
        (state.place != null && (state.place?.name.isNotEmpty ?? false)) ||
        (state.selectedUsers != null &&
            (state.selectedUsers?.isNotEmpty ?? false))) {
      return false;
    } else {
      return true;
    }
  ...
  onChangePage(int index) {
    ...
    emit(state.copyWith(
        ...
        selectedLifeEvent: LifeEventEntity(
            ...
            image: '',
            media: [],
            liveEventMainCategoryId: '',
            mainCat: LifeEventEntity(
                id: '',
                titleAr: '',
                titleEn: '',
                image: '',
  ...
  Future<void> onDiscardPost() async {
    ...
    emit(state.copyWith(
      ...
      selectedActivity:
          ActivityEntity(id: '', name: '', nameEn: '', image: '', mainId: ''),
      gifImage: '',
      backColor: '#FFFFFFFF',
      images: [],
      lifeEventImages: [],
  ...
  onRemoveActivity() {
    emit(state.copyWith(
        selectedActivity:
            ActivityEntity(name: '', nameEn: '', image: '', id: ''),
        status: CreatePostStates.success));
  }

  onRemoveAddress() {
    emit(
        state.copyWith(selectedLocation: '', status: CreatePostStates.success));
  }
  ...
  onSelectActivity(ActivityEntity activity) {
    emit(state.copyWith(
        selectedActivity: activity, status: CreatePostStates.success));
  }
  ...
  void selectActivity({required ActivityEntity item}) {
    emit(state.copyWith(
        ...
        selectedFeeling:
            FeelingEntity(id: '', name: '', nameEn: '', image: '')));
  }
