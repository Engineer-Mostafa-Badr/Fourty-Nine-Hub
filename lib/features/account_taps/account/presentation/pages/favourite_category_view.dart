        ...
        body: BlocBuilder<FavouriteCategoryCubit,FavouriteCategoryState>(
          builder: (context,state) {
            ...
            return state.status==StateStatus.loading?const Center(
              ...
              separatorBuilder: (context,i)=>const Sizer(height: 0,),
              itemBuilder: (context,i)=>state.data![i].id.isNotEmpty?Padding(
                padding: const EdgeInsets.only(bottom: 10.0),
                child: FavouriteMainCategoryBanner(category: state.data![i], canRegister: false, onFavorite: () async{
                  var result = await controller.removeFavorite(state.data![i].id);
                  if(result == true){
                    state.data?.removeWhere((element) => element.id==state.data![i].id);
                  }
                },),
