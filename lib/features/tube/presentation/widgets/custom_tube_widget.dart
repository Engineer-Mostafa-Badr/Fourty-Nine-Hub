    ...
    return Row(
      ...
      children: [
        ...
        IconButton(
          iconSize: 64,
          icon: Icon(
            playerController.value.isPlaying ? Icons.pause_circle_filled : Icons.play_circle_filled,
            color: Colors.white,
          ),
          onPressed: () {
            ...
            if (playerController.value.isPlaying && !_isDragging) _startHideTimer(playerController);
          },
        ),
        const SizedBox(width: 20),
        IconButton(
...
class _CustomVideoControlsState extends State<CustomVideoControls>
  ...
  Future<void> _initializePreviewController() async {
    if (_currentVideoUrl != widget.videoUrl) {
      ...
      try {
        ...
        debugPrint('Error initializing preview controller: $e');
        if (mounted) setState(() => _isPreviewReady = false);
      }
    }
  }
  ...
  void didUpdateWidget(CustomVideoControls oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.videoUrl != widget.videoUrl) _initializePreviewController();
  }
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<TubeCubit, TubeState>(
      bloc: widget.cubit, // استخدم الـ cubit الممرر
      builder: (context, state) {
        final chewieController = state.chewieController;
        final playerController = state.videoPlayerController;
        ...
        return GestureDetector(
          ...
          child: Stack(
            ...
            children: [
              ...
              if (_showControls) _buildControls(context, playerController, chewieController, duration, currentValue, position),
            ],
          ),
        );
      },
  ...
  Widget _centerControls(BuildContext context, VideoPlayerController playerController, ChewieController chewieController) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        IconButton(
          ...
          child: Row(
            ...
            children: [
              ...
              Row(
                children: [
                  IconButton(
                    icon: Icon(playerController.value.volume > 0 ? Icons.volume_up : Icons.volume_off,
                  ...
                  IconButton(
                    icon: Icon(chewieController.isFullScreen ? Icons.fullscreen_exit : Icons.fullscreen,
                        color: Colors.white, size: 24),
                    onPressed: chewieController.toggleFullScreen,
                  ),
                ],
