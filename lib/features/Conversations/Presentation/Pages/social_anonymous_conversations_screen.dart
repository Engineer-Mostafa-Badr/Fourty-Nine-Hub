...
class SocialAnonymousConversationsScreen extends StatefulWidget {
  const SocialAnonymousConversationsScreen({super.key});

  @override
  State<SocialAnonymousConversationsScreen> createState() => _SocialAnonymousConversationsScreenState();
}

class _SocialAnonymousConversationsScreenState extends State<SocialAnonymousConversationsScreen> {
    ...
    scrollController = ScrollController()
      ..addListener(() {
        if (scrollController.position.pixels >=
          ...
          serviceLocator<ConversationsCubit>().getSocialAnonymousConversations();
        }
      });
    super.initState();
  }
        ...
        child: BlocBuilder<ConversationsCubit, ConversationsState>(
            builder: (context, state) {
              if (state.status == ConversationsStates.loading &&
                  context.read<ConversationsCubit>().socialAnonymousConversations.isEmpty) {
                  ...
                  children: [
                    Expanded(
                      child: ListView.separated(
                        ...
                        // Enable scrolling
                        itemBuilder: (context, index) {
                          if(index == context.read<ConversationsCubit>().socialAnonymousConversations.length){
                            return Column(
                              children: [
                          ...
                          if(index >= context.read<ConversationsCubit>().socialAnonymousConversations.length + 1){
                            return Padding(
                              padding: EdgeInsets.only(bottom: 16.h, left: 16.w, right: 16.w),
                              child: ClipRRect(
                                borderRadius: BorderRadius.circular(12),
                          ...
                          return Slidable(
                            ...
                            child: ChatCard(
                              chat: context.read<ConversationsCubit>().socialAnonymousConversations[index],
                            ),
                          );
                        },
                        ...
                        itemCount: (context.read<ConversationsCubit>().socialAnonymousConversations.length + 1) +
                            ((context.read<ConversationsCubit>().isLoadingMoreSocialAnonymousConversation &&
                                context.read<ConversationsCubit>().socialAnonymousConversations.isNotEmpty)
                                ? 10
                                : 0),
                      ),
                    ),
