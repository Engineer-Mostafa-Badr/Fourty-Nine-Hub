          ...
          children: [
            ...
            state.status == StateStatus.loading
              ...
              child: Container(
                ...
                : Text(
              context.isArabic ? 'اختر نوع الاعلان' : 'Choose Ad Type',
              style: TextStyle(fontSize: FontSize.s18, color: context.isDarkMode ? Colors.white : AppColors.PRIMARY_COLOR, fontWeight: FontWeight.bold),
            ...
            _buildAdOption(context,
                ...
                image: Icons.image,
                title: context.isArabic ? 'اعلان صورة ونص' : 'Text & Photo Ad',
                description: context.isArabic ? 'أنشئ اعلان صورة ونص' : 'Create ads with text and static images',
                buttonText: context.isArabic ? 'أنشئ اعلان صورة ونص' : 'Create Photo and Text Ad', onPressed: () {
                  if (state.price?.isSubscribed == true) {
                    context.push(Routes.CREATECOMPANYPOSTAD,extra: CreatePostCompanyParams(
                        title: LocaleKeys.createPost.localize,
                        type: 'photo_written',
                        totalPrice: state.price?.postAndPhotoPrice ?? 0, text: true,
                        picture: true

                    ),);

                  } else if (state.price?.isSubscribed == false) {
                    SubscriptionMethod().subscribe(subscribeId: '', showRegular: true, title: LocaleKeys.companyAdvertise.localize);
                  }
                }),
            ...
            _buildAdOption(context,
                ...
                description: context.isArabic ? 'أنشئ اعلان فيديو' : 'Create short video ads',
                buttonText: context.isArabic ? 'أنشئ اعلان فيديو' : 'Create Reel Ad', onPressed: () {
                  if (state.price?.isSubscribed == true) {
                    context.push(Routes.CREATECOMPANYPOSTREALAD,extra: state.price?.reelPrice ?? 0,);
                  } else if (state.price?.isSubscribed == false) {
                    SubscriptionMethod().subscribe(subscribeId: Constants.companyAdsSubCategory, showRegular: true, title: LocaleKeys.companyAdvertise.localize);
                  }
                }),
          ],
        )
        ...
        child: Column(
          children: [
              ...
              child: Stack(
                ...
                children: [
                  ...
                  PositionedDirectional(
                    top: 10,
                    start: 10,
                    child: Icon(icon, size: 30, color: AppColors.SECONDARY_COLOR),
                  ),
                ],
              ),
            ),
            ...
            isLoading
              ...
              child: Text(
                buttonText,
                style: TextStyle(color: Colors.white, fontSize: FontSize.s14, fontWeight: FontWeight.bold),
              ),
            ),
          ],
        ),
